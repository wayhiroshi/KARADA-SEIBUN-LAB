...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "what-does-food-break-down-into", "food-to-building-blocks-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "what-does-food-break-down-into");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.995) 0%, rgba(251,247,239,.97) 45%, rgba(251,247,239,.30) 62%, rgba(251,247,239,0) 100%); }
  ...
  <section class="copy"><small>「分解される」の先を見る</small><h1>食べたものは、<br>体の中で何に分かれるの？</h1><p class="lead">ごはん、魚、油。<br>それぞれ違う小さな材料になります。</p><div class="answer"><strong>体が受け取れる大きさへ</strong></div></section>
  <div class="note">糖、アミノ酸、脂肪酸。食べ物の姿が変わります。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "what-does-food-break-down-into.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered what-does-food-break-down-into article hero.");
