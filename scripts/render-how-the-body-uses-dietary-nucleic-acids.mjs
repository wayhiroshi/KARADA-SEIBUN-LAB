...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "how-the-body-uses-dietary-nucleic-acids", "two-routes-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "how-the-body-uses-dietary-nucleic-acids");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.99) 0%, rgba(251,247,239,.97) 43%, rgba(251,247,239,.18) 65%, rgba(251,247,239,0) 100%); }
  ...
  .answer { display: inline-flex; align-items: center; gap: 14px; margin-top: 31px; padding: 17px 25px; border-radius: 18px; color: #fff; background: #7a8653; box-shadow: 0 12px 28px rgba(67,82,47,.15); }
  ...
  <section class="copy"><small>飲んだあとの行方をたどる</small><h1>食べた核酸は、<br>体の中でどう使われるの？</h1><p class="lead">一から作るだけではありません。<br>使える材料を回収する道もあります。</p><div class="answer"><strong>小さく分けて、再利用</strong></div></section>
  <div class="note">体には、材料を用意する二つの道がありました。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "how-the-body-uses-dietary-nucleic-acids.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered how-the-body-uses-dietary-nucleic-acids article hero.");
