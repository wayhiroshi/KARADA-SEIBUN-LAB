...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "nucleic-acids-in-everyday-foods", "table-foods-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "nucleic-acids-in-everyday-foods");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.99) 0%, rgba(251,247,239,.94) 42%, rgba(251,247,239,.18) 67%, rgba(251,247,239,0) 100%); }
  ...
  .copy { position: relative; z-index: 2; width: 700px; margin-top: 72px; }
  ...
  .answer { display: inline-block; margin-top: 31px; padding: 18px 24px; border-radius: 18px; color: #fff; background: #668b78; font-size: 25px; font-weight: 900; box-shadow: 0 12px 28px rgba(66,94,80,.14); }
  ...
  <section class="copy"><small>身近なところで見つける核酸</small><h1>核酸って、食品の中にも<br>最初から入っているの？</h1><p class="lead">特別な健康成分だと思っていたら、<br>いつもの食卓にもありました。</p><div class="answer">魚にも、大豆にも、DNAとRNAがある。</div></section>
  <div class="note">豆腐の原料である大豆も、細胞からできている。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "nucleic-acids-in-everyday-foods.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered nucleic-acids-in-everyday-foods article hero.");
