...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "where-are-nucleotide-materials-reused", "bodywide-nucleotide-reuse-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "where-are-nucleotide-materials-reused");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.995) 0%, rgba(251,247,239,.98) 42%, rgba(251,247,239,.54) 54%, rgba(251,247,239,0) 72%); }
  ...
  .copy { position: relative; z-index: 2; width: 650px; margin-top: 70px; }
  .copy small { color: #a95543; font-size: 27px; font-weight: 900; letter-spacing: .08em; }
  h1 { margin: 24px 0 0; color: #263d34; font-size: 58px; line-height: 1.28; letter-spacing: -.05em; }
  .lead { margin: 28px 0 0; color: #53645d; font-size: 27px; line-height: 1.62; font-weight: 600; }
  .answer { display: inline-flex; align-items: center; margin-top: 31px; padding: 17px 25px; border-radius: 18px; color: #fff; background: #7a8653; box-shadow: 0 12px 28px rgba(67,82,47,.15); }
  .answer strong { font-size: 30px; font-weight: 900; }
  ...
  <section class="copy"><small>肝臓の先まで見てみる</small><h1>核酸の材料を<br>再利用するのは、<br>肝臓だけ？</h1><p class="lead">材料を受け取って使う場所は、<br>もっと体じゅうにありました。</p><div class="answer"><strong>主役は、各地の細胞</strong></div></section>
  <div class="note">肝臓・腎臓・肺・小腸など、それぞれの場所で材料を使います。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "where-are-nucleotide-materials-reused.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered where-are-nucleotide-materials-reused article hero.");
