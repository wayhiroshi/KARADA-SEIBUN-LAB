...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "is-salacia-an-ingredient", "botanical-discovery-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "is-salacia-an-ingredient");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .copy { position: relative; z-index: 2; width: 720px; margin-top: 74px; }
  .copy small { color: #a95543; font-size: 27px; font-weight: 900; letter-spacing: .08em; }
  h1 { margin: 24px 0 0; color: #263d34; font-size: 59px; line-height: 1.25; letter-spacing: -.05em; }
  ...
  <section class="copy"><small>ダイエットと成分</small><h1>サラシアが<br>ダイエット茶に<br>入っているのは、なぜ？</h1><p class="lead">調べてみたら、植物だった。<br>その先に、糖質の消化がありました。</p><div class="answer">糖質を小さく分ける酵素との関係。</div></section>
  <div class="note">名前だけだった素材が、食事の流れにつながった。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "is-salacia-an-ingredient.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered is-salacia-an-ingredient article hero.");
