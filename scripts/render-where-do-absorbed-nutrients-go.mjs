...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "where-do-absorbed-nutrients-go", "nutrient-delivery-routes-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "where-do-absorbed-nutrients-go");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.995) 0%, rgba(251,247,239,.97) 45%, rgba(251,247,239,.28) 62%, rgba(251,247,239,0) 100%); }
  ...
  .copy { position: relative; z-index: 2; width: 670px; margin-top: 68px; }
  ...
  <section class="copy"><small>小腸の先をたどる</small><h1>吸収された栄養は、<br>体のどこへ行くの？</h1><p class="lead">小腸はゴールではなく発送口。<br>血液とリンパ、二つの道があります。</p><div class="answer"><strong>材料は体じゅうへ</strong></div></section>
  <div class="note">糖やアミノ酸は肝臓へ。脂質は別の道から合流します。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "where-do-absorbed-nutrients-go.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered where-do-absorbed-nutrients-go article hero.");
