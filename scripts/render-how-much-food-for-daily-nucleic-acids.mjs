...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "how-much-food-for-daily-nucleic-acids", "foods-for-1-5g-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "how-much-food-for-daily-nucleic-acids");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.99) 0%, rgba(251,247,239,.96) 43%, rgba(251,247,239,.2) 65%, rgba(251,247,239,0) 100%); }
  ...
  .copy { position: relative; z-index: 2; width: 690px; margin-top: 70px; }
  .copy small { color: #a95543; font-size: 27px; font-weight: 900; letter-spacing: .08em; }
  h1 { margin: 24px 0 0; color: #263d34; font-size: 57px; line-height: 1.28; letter-spacing: -.05em; }
  .lead { margin: 28px 0 0; color: #53645d; font-size: 27px; line-height: 1.62; font-weight: 600; }
  .answer { display: inline-flex; align-items: baseline; gap: 14px; margin-top: 31px; padding: 17px 25px; border-radius: 18px; color: #fff; background: #7a8653; box-shadow: 0 12px 28px rgba(67,82,47,.15); }
  ...
  <section class="copy"><small>食品のDNAを数字で見る</small><h1>DNAを1.5g集めるなら、<br>どのくらい食べる？</h1><p class="lead">きのこ、豆腐、魚、肉、野菜。<br>同じ物差しで比べてみました。</p><div class="answer"><span>生のきのこでも</span><strong>約700g</strong></div></section>
  <div class="note">生の玉ねぎなら約25kg。食卓がいっぱいになります。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "how-much-food-for-daily-nucleic-acids.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered how-much-food-for-daily-nucleic-acids article hero.");
