...
const data = JSON.parse(await readFile(path.join(root, "content", "social", "why-nucleotides-formula-2026-08-13.json"), "utf8"));
const brandMark = (await readFile(path.join(root, "public", "assets", "brand", "karada-seibun-lab-mark.svg"))).toString("base64");
const comicPath = path.join(root, "public", "assets", "source-illustrations", "why-nucleotides-formula", "comic-sheet.png");
const articleOutput = path.join(root, "public", "assets", "articles", "why-nucleotides-in-infant-formula");
...
const flowCard = `
  <div class="ingredient-card flow-card">
    <div><small>母乳を調べる</small><strong>成分を知る</strong></div>
    <i>→</i>
    <div><small>粉ミルクづくり</small><strong>研究を生かす</strong></div>
  </div>`;

const visualFor = (name) => name === "flow"
  ? `<div class="scene panel-3"></div>${flowCard}`
...
const baseCss = `
  ...
  * { box-sizing: border-box; }
  ...
  .flow-card { flex-direction: row; align-items: center; gap: 18px; padding: 30px; }
  .flow-card div { flex: 1; padding: 23px 16px; border-radius: 22px; text-align: center; background: #edf5ef; }
  .flow-card div:last-child { background: #fff1e8; }
  .flow-card small { display: block; font-size: 17px; }
  .flow-card strong { display: block; margin-top: 8px; color: #294f43; font-size: 21px; }
  .flow-card i { color: #bd624d; font-size: 36px; font-style: normal; font-weight: 900; }
`;
...
const articleHtml = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>${articleCss}</style></head><body><main class="hero">
  ...
  <section class="copy"><small>成分表から生まれた疑問</small><h1>なぜ、粉ミルクに<br>ヌクレオチドが<br>入っているの？</h1><p class="lead">答えは、<br>母乳の成分研究にありました。</p><div class="answer">研究の成果を、商品づくりへ。</div></section>
  <section class="hero-visual" aria-label="母乳の成分研究を粉ミルクの設計へつなげる筆者の水彩漫画"><div class="scene panel-3"></div></section>
  <div class="source-note">成分表の小さな文字から、研究の歴史をたどります。</div>
</main></body></html>`;

await renderPage({ html: articleHtml, output: path.join(articleOutput, "why-nucleotides-in-infant-formula.png"), width: 1440, height: 960 });
...
await browser.close();
console.log(`Rendered why-nucleotides/formula article hero and ${data.slides.length} social slides.`);
