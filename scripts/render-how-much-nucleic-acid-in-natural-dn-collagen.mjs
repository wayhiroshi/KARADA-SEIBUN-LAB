...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "how-much-nucleic-acid-in-natural-dn-collagen", "three-glasses-and-salmon-v2.png");
const outputDir = path.join(root, "public", "assets", "articles", "how-much-nucleic-acid-in-natural-dn-collagen");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.99) 0%, rgba(251,247,239,.96) 43%, rgba(251,247,239,.18) 67%, rgba(251,247,239,0) 100%); }
  ...
  .copy { position: relative; z-index: 2; width: 675px; margin-top: 66px; }
  .copy small { color: #a95543; font-size: 26px; font-weight: 900; letter-spacing: .08em; }
  h1 { margin: 24px 0 0; color: #263d34; font-size: 55px; line-height: 1.28; letter-spacing: -.05em; }
  .lead { margin: 26px 0 0; color: #53645d; font-size: 27px; line-height: 1.62; font-weight: 600; }
  .answer { display: inline-flex; align-items: baseline; gap: 14px; margin-top: 30px; padding: 17px 25px; border-radius: 18px; color: #fff; background: #a95543; box-shadow: 0 12px 28px rgba(100,62,52,.15); }
  .answer span { font-size: 22px; font-weight: 800; }
  .answer strong { font-size: 42px; font-weight: 900; }
  ...
  <section class="copy"><small>いつもの60mlを計算</small><h1>核酸ドリンクには、<br>DNAとRNAが<br>どのくらい？</h1><p class="lead">一日60ml。<br>商品表示の6割で計算しました。</p><div class="answer"><span>DNA＋RNA</span><strong>216mg</strong></div></section>
  <div class="note">DNAだけなら、サーモン約440g分。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "how-much-nucleic-acid-in-natural-dn-collagen-v3.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered how-much-nucleic-acid-in-natural-dn-collagen article hero.");
