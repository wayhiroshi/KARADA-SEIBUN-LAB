...
const heroSource = path.join(root, "public", "assets", "source-illustrations", "how-much-dna-in-fish", "fish-dna-amount-v1.png");
const outputDir = path.join(root, "public", "assets", "articles", "how-much-dna-in-fish");
...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero::before { content: ""; position: absolute; inset: 0; background: linear-gradient(90deg, rgba(251,247,239,.99) 0%, rgba(251,247,239,.96) 43%, rgba(251,247,239,.18) 65%, rgba(251,247,239,0) 100%); }
  ...
  .copy { position: relative; z-index: 2; width: 720px; margin-top: 78px; }
  .copy small { color: #a95543; font-size: 27px; font-weight: 900; letter-spacing: .08em; }
  h1 { margin: 24px 0 0; color: #263d34; font-size: 58px; line-height: 1.25; letter-spacing: -.05em; }
  .lead { margin: 28px 0 0; color: #53645d; font-size: 28px; line-height: 1.62; font-weight: 600; }
  .answer { display: inline-flex; align-items: baseline; gap: 14px; margin-top: 31px; padding: 17px 25px; border-radius: 18px; color: #fff; background: #567b8f; box-shadow: 0 12px 28px rgba(61,88,104,.14); }
  .answer span { font-size: 22px; font-weight: 800; }
  .answer strong { font-size: 38px; font-weight: 900; }
  .note { position: absolute; z-index: 2; left: 80px; bottom: 66px; color: #60716b; font-size: 20px; font-weight: 800; letter-spacing: .02em; }
  .sample-label { position: absolute; z-index: 2; right: 100px; top: 110px; padding: 12px 20px; border-radius: 999px; background: rgba(255,255,255,.92); color: #29465a; font-size: 22px; font-weight: 900; box-shadow: 0 8px 24px rgba(34,58,73,.1); }
</style></head><body><main class="hero">
  <div class="brand"><img src="data:image/svg+xml;base64,${brandMark}" alt="">からだ成分ラボ</div>
  <section class="copy"><small>身近な食品の核酸</small><h1>魚のDNAって、<br>どのくらい入っているの？</h1><p class="lead">生の魚100gから、<br>研究で取り出せた量を見てみました。</p><div class="answer"><span>サーモン100gで</span><strong>約33mg</strong></div></section>
  <div class="sample-label">研究で抽出したDNA</div>
  <div class="note">生の白身魚100gでは約26mgでした。</div>
</main></body></html>`;
...
await page.screenshot({ path: path.join(outputDir, "how-much-dna-in-fish.png"), type: "png" });
await page.close();
await browser.close();
console.log("Rendered how-much-dna-in-fish article hero.");
